...
class Fetcher(object):
    ...
    # add all hoster may streamkiste/s.to + 1 movie/serie stream site  + cine
    ...
    def Crawl_Seasons(self, serie):
        ...
        for season in seasonsDiv:
            link = "http://bs.to/" + season.find("a").get('href')
    ...
    def getWork(self):
        ...
        for serienData in serienDataList:
            ...
            SeasonIds = db.select(
                table="Staffel", select="ID", where="`serien_id` = '" + serienId + "'")
           # if(len(SeasonIds) < 1):
